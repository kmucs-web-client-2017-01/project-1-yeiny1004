...
function zoomInAndOut(string) { 
  ...
  if (string == "in") { 
    var width = parseInt(currentWidth * 1.1); 
    var height = parseInt(currentHeight * 1.1);  
  } 
  else if (string == "out") { 
    var width = parseInt(currentWidth * 0.9);  
    var height = parseInt(currentHeight * 0.9);  
  } 
  image.width = width; 
  image.height = height; 
}
...
$('.rabbit img').bind('click', function(){
        ...
        else $(this).attr('src', 'rabbit2.gif'); 
    });
